...
function TrinketMenu.LoadDefaults()
	...
	TrinketMenuOptions = TrinketMenuOptions or {
		...
		ForceShow = "ON",			-- keep main frame visible and self-heal if hidden by other addons
	}
...
function TrinketMenu.Initialize()
	...
	options.ForceShow = options.ForceShow or "ON"
	...
	TrinketMenu.CreateTimer("AntiHide",TrinketMenu.AntiHide,.5,1)
	...
	-- start anti-hide self-healing if enabled
	if TrinketMenuOptions.ForceShow=="ON" then
		TrinketMenu.StartTimer("AntiHide")
	end
	...
	-- ensure main frame is not hidden behind other frames
	TrinketMenu_MainFrame:SetFrameStrata("MEDIUM")
...
function TrinketMenu.SlashHandler(msg)
	...
	elseif msg=="show" then
		TrinketMenu._userHide = nil
		TrinketMenuPerOptions.Visible = "ON"
		if not InCombatLockdown() then
			TrinketMenu_MainFrame:Show()
		else
			DEFAULT_CHAT_FRAME:AddMessage("|cFFFFFF00TrinketMenu: cannot show while in combat.")
		end
	elseif msg=="hide" then
		TrinketMenu._userHide = 1
		TrinketMenuPerOptions.Visible = "OFF"
		if not InCombatLockdown() then
			TrinketMenu_MainFrame:Hide()
		else
			DEFAULT_CHAT_FRAME:AddMessage("|cFFFFFF00TrinketMenu: cannot hide while in combat.")
		end
	elseif msg=="reset" then
		TrinketMenu.ResetSettings()
	elseif msg=="fix" then
		TrinketMenu.FixFrames()
	...
	elseif string.find(msg,"alwayson") then
		local _,_,state = string.find(msg,"alwayson (.+)")
		if state then
			state = string.lower(state)
			if state=="on" or state=="1" or state=="true" then
				TrinketMenuOptions.ForceShow = "ON"
				TrinketMenu.StartTimer("AntiHide")
				DEFAULT_CHAT_FRAME:AddMessage("|cFFFFFF00TrinketMenu AlwaysOn: ON")
			elseif state=="off" or state=="0" or state=="false" then
				TrinketMenuOptions.ForceShow = "OFF"
				TrinketMenu.StopTimer("AntiHide")
				DEFAULT_CHAT_FRAME:AddMessage("|cFFFFFF00TrinketMenu AlwaysOn: OFF")
			else
				DEFAULT_CHAT_FRAME:AddMessage("|cFFFFFF00Usage: /trinket alwayson on|off")
			end
		else
			DEFAULT_CHAT_FRAME:AddMessage("|cFFFFFF00Usage: /trinket alwayson on|off")
		end
	elseif string.find(msg,"load") then
		...
		DEFAULT_CHAT_FRAME:AddMessage("/trinket show|hide : explicitly show or hide TrinketMenu")
		DEFAULT_CHAT_FRAME:AddMessage("/trinket fix : re-show and repair position/strata/alpha if hidden by another addon")
		DEFAULT_CHAT_FRAME:AddMessage("/trinket alwayson on|off : keep window visible even if hidden by another addon")
	end
end
...
function TrinketMenu.OnHide()
	-- always hide the menu with the main frame
	TrinketMenu_MenuFrame:Hide()
	-- if the user explicitly hid the frame (via toggle/hide), respect that
	if TrinketMenu._userHide then
		TrinketMenuPerOptions.Visible = "OFF"
	else
		-- likely hidden by another addon; if allowed, reshow shortly
		if TrinketMenuOptions.ForceShow=="ON" and not InCombatLockdown() then
			TrinketMenu.StartTimer("AntiHide",0.1)
		end
	end
end
...
--[[ Anti-hide and quick fix ]]

function TrinketMenu.AntiHide()
	if TrinketMenuOptions.ForceShow~="ON" then return end
	-- do not attempt protected changes in combat
	if InCombatLockdown() then return end
	-- only keep visible if the user intends it to be visible
	if TrinketMenuPerOptions.Visible=="ON" then
		-- ensure parent is UIParent
		if TrinketMenu_MainFrame:GetParent()~=UIParent then
			TrinketMenu_MainFrame:SetParent(UIParent)
		end
		-- ensure strata/levels are sane
		TrinketMenu_MainFrame:SetFrameStrata("MEDIUM")
		TrinketMenu_MainFrame:SetFrameLevel(1)
		TrinketMenu_Trinket0:SetFrameLevel(2)
		TrinketMenu_Trinket1:SetFrameLevel(2)
		-- ensure alpha/scale are not zeroed out
		if (TrinketMenu_MainFrame:GetAlpha() or 1) < 0.1 then
			TrinketMenu_MainFrame:SetAlpha(TrinketMenuPerOptions.Alpha or 1)
		end
		if (TrinketMenu_MainFrame:GetScale() or 1) < 0.1 then
			TrinketMenu_MainFrame:SetScale(TrinketMenuPerOptions.MainScale or 1)
		end
		-- ensure it's actually shown
		if not TrinketMenu_MainFrame:IsShown() or not TrinketMenu_MainFrame:IsVisible() then
			TrinketMenu_MainFrame:Show()
		end
	end
end

function TrinketMenu.FixFrames()
	if InCombatLockdown() then
		DEFAULT_CHAT_FRAME:AddMessage("|cFFFFFF00TrinketMenu: cannot fix while in combat.")
		return
	end
	TrinketMenu._userHide = nil
	TrinketMenuPerOptions.Visible = "ON"
	...
	TrinketMenu_MainFrame:SetPoint("TOPLEFT","UIParent","BOTTOMLEFT",TrinketMenuPerOptions.XPos or 400,TrinketMenuPerOptions.YPos or 400)
	TrinketMenu_MainFrame:SetParent(UIParent)
	TrinketMenu_MainFrame:SetFrameStrata("MEDIUM")
	TrinketMenu_MainFrame:SetFrameLevel(1)
	TrinketMenu_Trinket0:SetFrameLevel(2)
	TrinketMenu_Trinket1:SetFrameLevel(2)
	TrinketMenu_MainFrame:SetScale(TrinketMenuPerOptions.MainScale or 1)
	TrinketMenu_MenuFrame:SetScale(TrinketMenuPerOptions.MenuScale or 1)
	TrinketMenu_MainFrame:SetAlpha(TrinketMenuPerOptions.Alpha or 1)
	TrinketMenu.OrientWindows()
	TrinketMenu.DockWindows()
	TrinketMenu_MainFrame:Show()
	...
	DEFAULT_CHAT_FRAME:AddMessage("|cFFFFFF00TrinketMenu: repaired and shown.")
end
